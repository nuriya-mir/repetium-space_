...
type Segment =
  ...
  | { type: 'target'; stem: string; ending: string; endingType: string; color: string; ttsText: string; idx: number }

// ── Constants ─────────────────────────────────────────────────────────────────
const ENDING_COLORS = ['#D4AA38', '#7A9AB8', '#C06A30', '#4EAA78']

const ENDING_DEFS = [
  { key: '-tion',       suffixes: ['tion'],         example: 'information', label: '-tion' },
  { key: '-ment',       suffixes: ['ment'],         example: 'doucement',   label: '-ment' },
  { key: '-eux',        suffixes: ['eux'],           example: 'heureux',     label: '-eux' },
  { key: '-eur',        suffixes: ['eur'],           example: 'douceur',     label: '-eur' },
  { key: '-oir',        suffixes: ['oir'],           example: 'espoir',      label: '-oir' },
  { key: '-age',        suffixes: ['age'],           example: 'voyage',      label: '-age' },
  { key: '-ette',       suffixes: ['ette'],          example: 'recette',     label: '-ette' },
  { key: '-ance/-ence', suffixes: ['ance', 'ence'], example: 'confiance',   label: '-ance/-ence' },
  { key: '-ible/-able', suffixes: ['ible', 'able'], example: 'possible',    label: '-ible/-able' },
] as const

const HINT_AFTER_ERRORS = 3

// ── Segment parser ────────────────────────────────────────────────────────────
function parseG10Segments(text: string, selectedKeys: string[]): Segment[] {
  // Build active suffix list with metadata
  const active: Array<{ suffix: string; key: string; color: string }> = []
  selectedKeys.forEach((key, colorIdx) => {
    const def = ENDING_DEFS.find(d => d.key === key)
    if (!def) return
    def.suffixes.forEach(suffix => {
      active.push({ suffix, key, color: ENDING_COLORS[colorIdx % ENDING_COLORS.length] })
    })
  })
  // Sort longest first for priority matching
  active.sort((a, b) => b.suffix.length - a.suffix.length)
  ...
  for (const token of tokens) {
    ...
    for (const { suffix, key, color } of active) {
      if (lower.endsWith(suffix) && token.length - suffix.length >= 2) {
        const stemLen = token.length - suffix.length
        segments.push({
          type: 'target',
          stem: token.slice(0, stemLen),
          ending: token.slice(stemLen),
          endingType: key,
          color,
          ttsText: suffix,
          idx: targetIdx++,
...
// ── Main component ────────────────────────────────────────────────────────────
export default function G10Page() {
  const router = useRouter()
  const t = useT()
  useTrackExercise('g10', 'Окончания слов', '/exercises/g10')
  ...
  const allFilled = useMemo(
    () => targets.length > 0 && targets.every((tgt, i) => values[i] === tgt.ending),
    [targets, values]
  )
  ...
  // ── Auto-play unique morpheme sounds on show ──────────────────────────────
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (const key of selectedKeys) {
        ...
        const def = ENDING_DEFS.find(d => d.key === key)
        if (def) await playTTS(def.suffixes[0])
  ...
  // ── Toggle ending selection ───────────────────────────────────────────────
  function toggleEnding(key: string) {
  ...
  async function loadText() {
    ...
    try {
      const res = await fetch('/api/generate-text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ group_id: 'g10', letter_targets: selectedKeys }),
      })
      const data = await res.json()
      const text: string = data.content ?? ''
      if (!text) throw new Error('empty')

      const segs = parseG10Segments(text, selectedKeys)
  ...
  const handleKeyInput = useCallback((idx: number, char: string) => {
    ...
    setValues(prev => {
      ...
      if (!target || currentTyped === target.ending) return prev

      const expectedChar = target.ending[currentTyped.length]
      ...
      if (correct) {
        ...
        if (newTyped === target.ending) {
          // Fully filled
          playTTS(target.ttsText).catch(() => {})
          setTimeout(() => {
            const nextIdx = next.findIndex((v, i) => i > idx && targets[i] && v !== targets[i].ending)
  ...
  // Legend for 2+ ending types
  const legend = selectedKeys.length > 1 && (
    <div className={styles.legend}>
      {selectedKeys.map((key, i) => (
        <span key={key} className={styles.legendItem}>
          <span className={styles.legendDot} style={{ background: ENDING_COLORS[i % ENDING_COLORS.length] }} />
    ...
    return (
      <div className={styles.page}>
        <div className={styles.topbar}>
          {closeBtn}
          <span className={styles.groupLabel}>{t.exercises.g10name}</span>
          {formatToggle}
        </div>
        <div className={styles.selectWrap}>
          <p className={styles.selectPrompt}>{t.exercises.g10selectHint}</p>
          <p className={styles.selectedCount}>
            {t.exercises.g10selectedCount.replace('{count}', String(selectedKeys.length))}
          </p>
          ...
          <div className={styles.endingGrid}>
            {ENDING_DEFS.map(def => {
              ...
              const color = sel ? ENDING_COLORS[selIdx % ENDING_COLORS.length] : undefined
              return (
                <button
                  key={def.key}
                  className={`${styles.endingCard} ${sel ? styles.endingCardSelected : ''}`}
                  style={sel ? { borderColor: color, background: color + '33' } : undefined}
                  onClick={() => toggleEnding(def.key)}
                  disabled={!sel && selectedKeys.length >= 4}
                >
                  <span className={styles.endingLabel}>{def.label}</span>
                  <span className={styles.endingExample}>{def.example}</span>
                </button>
              )
            })}
          </div>
  ...
  if (phase === 'loading') {
    return (
      <div className={styles.page}>
        <div className={styles.topbar}>{closeBtn}<span className={styles.groupLabel}>{t.exercises.g10name}</span>{formatToggle}</div>
  ...
  if (phase === 'complete') {
    return (
      <div className={styles.page}>
        <div className={styles.topbar}><span className={styles.groupLabel}>{t.exercises.g10name}</span>{formatToggle}</div>
        <div className={styles.completeWrap}>
          ...
          <p className={`${styles.exerciseText} ${format === 'adapted' ? styles.exerciseTextAdapted : ''}`}>
            {segments.map((seg, i) => {
              ...
              return (
                <span key={i}>
                  <span>{seg.stem}</span>
                  <span style={{ color: seg.color, fontWeight: 600 }}>{seg.ending}</span>
                </span>
              )
            })}
          </p>
          <div className={styles.completeActions}>
            ...
            <button className={styles.actionBtn} onClick={restart}>{t.exercises.g10anotherEl}</button>
  ...
  // ── SHOW / FADE / RECALL ──────────────────────────────────────────────────
  const filledCount = values.filter((v, i) => v === targets[i]?.ending).length
      ...
      <div className={styles.exerciseWrap}>
        ...
        <p className={`${styles.exerciseText} ${format === 'adapted' ? styles.exerciseTextAdapted : ''}`}>
          {segments.map((seg, i) => {
            ...
            const showHint = hints.has(idx) && typed !== seg.ending
            const fullyFilled = typed === seg.ending
            ...
            if (phase === 'show') {
              return (
                <span key={i}>
                  <span
                    className={styles.wordWithEnding}
                    onClick={() => playTTS(seg.stem + seg.ending)}
                  >
                    {seg.stem}
                  </span>
                  <span
            ...
            if (phase === 'fade') {
              return (
                <span key={i}>
                  <span>{seg.stem}</span>
                  {fadeStep === 0 && <span className={styles.highlight} style={{ background: bgColor }}>{seg.ending}</span>}
                  {fadeStep === 1 && <span className={styles.fadeDots}>{'•'.repeat(seg.ending.length)}</span>}
                  {fadeStep === 2 && <span className={styles.fadeEmpty}>{'\u00A0'.repeat(seg.ending.length)}</span>}
                </span>
              )
            }
            ...
            if (fullyFilled) {
              return (
                <span key={i}>
                  <span>{seg.stem}</span>
                  <span className={styles.filledEnding} style={{ color: seg.color }}>{typed}</span>
                </span>
              )
            }

            const remaining = seg.ending.slice(typed.length)
            return (
        ...
        {phase === 'show' && (
          <div className={styles.bottomBar}>
            <p className={styles.showHint}>{t.exercises.g10showHint}</p>
            <button className={styles.readyBtn} onClick={startFade}>{t.exercises.g10readyBtn}</button>
          </div>
        )}
