...
type Segment =
  ...
  | { type: 'target'; stem: string; silent: string; silentType: string; color: string; word: string; idx: number }
...
const EXCEPTIONS: Record<string, Set<string>> = {
  t: new Set(['net', 'sept', 'ouest', 'brut', 'but', 'scout', 'kit', 'bit', 'dot', 'mat']),
  s: new Set(['bus', 'tennis', 'mars', 'fils', 'sens', 'iris', 'oasis', 'atlas', 'as', 'bis', 'lis']),
  d: new Set(['sud']),
  x: new Set(['index', 'latex', 'phoenix']),
  e: new Set(),
  p: new Set(['cap', 'stop']),
  z: new Set(['gaz']),
}
...
// ── Segment parser ────────────────────────────────────────────────────────────
function parseG7Segments(text: string, selectedKeys: string[]): Segment[] {
  // Build colorMap: key → color (by index in selectedKeys)
  ...
  for (const token of tokens) {
    ...
    const lastChar = token[token.length - 1].toLowerCase()

    if (selectedKeys.includes(lastChar)) {
      const lower = token.toLowerCase()

      // Check exceptions
      const excSet = EXCEPTIONS[lastChar]
      if (excSet && excSet.has(lower)) {
        segments.push({ type: 'text', content: token })
        continue
      }

      // Special 'e' check: actual last char must not be accented é, è, ê, ë
      if (lastChar === 'e') {
        const actualLast = token[token.length - 1]
        if ('éèêë'.includes(actualLast)) {
          segments.push({ type: 'text', content: token })
          continue
        }
      }
      ...
      const silent = token[token.length - 1]
      segments.push({
        type: 'target',
        stem,
        silent,
        silentType: lastChar,
        color: colorMap[lastChar],
        word: token,
        idx: targetIdx++,
      })
    } else {
...
export default function G7Page() {
  ...
  const t = useT()
  useTrackExercise('g7', 'Немые буквы', '/exercises/g7')
  ...
  // ── Auto-play all target words on show (silence = 300ms pause, then word) ─
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (const tgt of targets) {
        ...
        // "silence" — the letter has no sound, just a short pause
  ...
  // ── Toggle silent letter selection ───────────────────────────────────────
  function toggleSilent(key: string) {
  ...
  async function loadText() {
    ...
    try {
      const res = await fetch('/api/generate-text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ group_id: 'g7', letter_targets: selectedKeys }),
      })
      const data = await res.json()
      const text: string = data.content ?? ''
      if (!text) throw new Error('empty')

      const segs = parseG7Segments(text, selectedKeys)
  ...
  // ── Input handling (single character) ────────────────────────────────────
  const handleKeyInput = useCallback((idx: number, char: string) => {
    ...
    setValues(prev => {
      ...
      if (correct) {
        const next = [...prev]
        next[idx] = target.silent // store original case
  ...
  // Legend for 2+ silent letter types
    ...
    return (
      <div className={styles.page}>
        <div className={styles.topbar}>
          {closeBtn}
          <span className={styles.groupLabel}>{t.exercises.g7name}</span>
          {formatToggle}
        </div>
        <div className={styles.selectWrap}>
          <p className={styles.selectPrompt}>{t.exercises.g7selectHint}</p>
          <p className={styles.selectedCount}>
            {t.exercises.g7selectedCount.replace('{count}', String(selectedKeys.length))}
          </p>
            ...
            {SILENT_DEFS.map(def => {
              ...
              return (
                <button
                  ...
                  onClick={() => toggleSilent(def.key)}
                  disabled={!sel && selectedKeys.length >= 4}
                >
                  <span className={styles.silentLabel}>{def.key}</span>
  ...
  if (phase === 'loading') {
    return (
      <div className={styles.page}>
        <div className={styles.topbar}>{closeBtn}<span className={styles.groupLabel}>{t.exercises.g7name}</span>{formatToggle}</div>
  ...
  if (phase === 'complete') {
    return (
      <div className={styles.page}>
        <div className={styles.topbar}><span className={styles.groupLabel}>{t.exercises.g7name}</span>{formatToggle}</div>
        <div className={styles.completeWrap}>
          ...
          <div className={styles.completeActions}>
            ...
            <button className={styles.actionBtn} onClick={restart}>{t.exercises.g7anotherEl}</button>
      ...
      <div className={styles.exerciseWrap}>
          ...
          {segments.map((seg, i) => {
            ...
            if (phase === 'show') {
              return (
                <span key={i}>
                  <span
                    className={styles.wordStem}
                    onClick={() => playTTS(seg.word)}
                  >
                    {seg.stem}
                  </span>
                  <span
        ...
        {phase === 'show' && (
          <div className={styles.bottomBar}>
            <p className={styles.showHint}>{t.exercises.g7showHint}</p>
            <button className={styles.readyBtn} onClick={startFade}>{t.exercises.g7readyBtn}</button>
          </div>
        )}

        {phase === 'recall' && (
          <div className={styles.bottomBar}>
            <p className={styles.showHint}>{t.exercises.g7recallHint}</p>
            <p className={styles.filledCount}>
              {t.exercises.g7filledCount
