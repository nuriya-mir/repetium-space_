...
type Segment =
  ...
  | { type: 'target'; beforeNasal: string; nasal: string; afterNasal: string; nasalType: string; color: string; idx: number }

// ── Constants ─────────────────────────────────────────────────────────────────
const NASAL_COLORS = ['#D4AA38', '#7A9AB8', '#C06A30', '#4EAA78']

const NASAL_DEFS = [
  { key: 'an',  ipa: '[ɑ̃]',  example: 'France' },
  { key: 'en',  ipa: '[ɑ̃]',  example: 'moment' },
  { key: 'on',  ipa: '[ɔ̃]',  example: 'bonjour' },
  { key: 'in',  ipa: '[ɛ̃]',  example: 'jardin' },
  { key: 'un',  ipa: '[œ̃]',  example: 'lundi' },
  { key: 'ain', ipa: '[ɛ̃]',  example: 'pain' },
  { key: 'oin', ipa: '[wɛ̃]', example: 'point' },
] as const

const VOWELS = new Set('aeiouàâéèêëîïôùûüÿ')
const HINT_AFTER_ERRORS = 3

// ── Segment parser ────────────────────────────────────────────────────────────
function parseG4Segments(text: string, selectedKeys: string[]): Segment[] {
  // Build active nasal list with metadata, sorted longest first
  const active: Array<{ nasal: string; key: string; color: string }> = []
  selectedKeys.forEach((key, colorIdx) => {
    const def = NASAL_DEFS.find(d => d.key === key)
    if (!def) return
    active.push({ nasal: key, key, color: NASAL_COLORS[colorIdx % NASAL_COLORS.length] })
  })
  active.sort((a, b) => b.nasal.length - a.nasal.length)
  ...
  for (const token of tokens) {
    ...
    for (const { nasal, key, color } of active) {
      const idx = lower.indexOf(nasal)
      if (idx === -1) continue

      // Check it's a real nasal (not followed by a vowel or doubled consonant)
      const nextCharIdx = idx + nasal.length
      if (nextCharIdx < lower.length) {
        const nextChar = lower[nextCharIdx]
        // Followed by a vowel → not nasal
        if (VOWELS.has(nextChar)) continue
        // Doubled consonant (nextChar same as last char of nasal) → not nasal
        if (nextChar === nasal[nasal.length - 1]) continue
      }

      segments.push({
        type: 'target',
        beforeNasal: token.slice(0, idx),
        nasal: token.slice(idx, idx + nasal.length),
        afterNasal: token.slice(idx + nasal.length),
        nasalType: key,
        color,
        idx: targetIdx++,
      })
...
export default function G4Page() {
  ...
  const t = useT()
  useTrackExercise('g4', 'Носовые звуки', '/exercises/g4')
  ...
  const allFilled = useMemo(
    () => targets.length > 0 && targets.every((tgt, i) => values[i]?.toLowerCase() === tgt.nasal.toLowerCase()),
    [targets, values]
  )
  ...
  // ── Auto-play nasal sounds on show ────────────────────────────────────────
  useEffect(() => {
  ...
  // ── Toggle nasal selection ────────────────────────────────────────────────
  function toggleNasal(key: string) {
  ...
  async function loadText() {
    ...
    try {
      const res = await fetch('/api/generate-text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ group_id: 'g4', letter_targets: selectedKeys }),
      })
      const data = await res.json()
      const text: string = data.content ?? ''
      if (!text) throw new Error('empty')

      const segs = parseG4Segments(text, selectedKeys)
  ...
  const handleKeyInput = useCallback((idx: number, char: string) => {
    ...
    setValues(prev => {
      ...
      if (!target || currentTyped.toLowerCase() === target.nasal.toLowerCase()) return prev

      const expectedChar = target.nasal[currentTyped.length]
      ...
      if (correct) {
        ...
        if (newTyped.toLowerCase() === target.nasal.toLowerCase()) {
          // Fully filled
          playTTS(target.nasalType).catch(() => {})
          setTimeout(() => {
            const nextIdx = next.findIndex((v, i) => i > idx && targets[i] && v.toLowerCase() !== targets[i].nasal.toLowerCase())
      ...
      {selectedKeys.map((key, i) => {
        ...
        return (
          <span key={key} className={styles.legendItem}>
            <span className={styles.legendDot} style={{ background: NASAL_COLORS[i % NASAL_COLORS.length] }} />
    ...
    return (
      <div className={styles.page}>
        <div className={styles.topbar}>
          {closeBtn}
          <span className={styles.groupLabel}>{t.exercises.g4name}</span>
          {formatToggle}
        </div>
        <div className={styles.selectWrap}>
          <p className={styles.selectPrompt}>{t.exercises.g4selectHint}</p>
          <p className={styles.selectedCount}>
            {t.exercises.g4selectedCount.replace('{count}', String(selectedKeys.length))}
          </p>

          {errorMsg && <p className={styles.errorMsg}>{errorMsg}</p>}

          <div className={styles.nasalGrid}>
            {NASAL_DEFS.map(def => {
              const sel = selectedKeys.includes(def.key)
              const selIdx = selectedKeys.indexOf(def.key)
              const color = sel ? NASAL_COLORS[selIdx % NASAL_COLORS.length] : undefined
              return (
                <button
                  key={def.key}
                  className={`${styles.nasalCard} ${sel ? styles.nasalCardSelected : ''}`}
                  style={sel ? { borderColor: color, background: color + '33' } : undefined}
                  onClick={() => toggleNasal(def.key)}
                  disabled={!sel && selectedKeys.length >= 4}
                >
                  <span className={styles.nasalKey}>{def.key}</span>
                  <span className={styles.nasalExample}>{def.example}</span>
                  <span className={styles.nasalIpa}>{def.ipa}</span>
                </button>
              )
            })}
          </div>
  ...
  if (phase === 'loading') {
    return (
      <div className={styles.page}>
        <div className={styles.topbar}>{closeBtn}<span className={styles.groupLabel}>{t.exercises.g4name}</span>{formatToggle}</div>
  ...
  if (phase === 'complete') {
    return (
      <div className={styles.page}>
        <div className={styles.topbar}><span className={styles.groupLabel}>{t.exercises.g4name}</span>{formatToggle}</div>
        <div className={styles.completeWrap}>
          ...
          <p className={`${styles.exerciseText} ${format === 'adapted' ? styles.exerciseTextAdapted : ''}`}>
            {segments.map((seg, i) => {
              ...
              return (
                <span key={i}>
                  <span>{seg.beforeNasal}</span>
                  <span style={{ color: seg.color, fontWeight: 600 }}>{seg.nasal}</span>
                  <span>{seg.afterNasal}</span>
                </span>
              )
            })}
          </p>
          <div className={styles.completeActions}>
            ...
            <button className={styles.actionBtn} onClick={restart}>{t.exercises.g4anotherEl}</button>
  ...
  // ── SHOW / FADE / RECALL ──────────────────────────────────────────────────
  const filledCount = values.filter((v, i) => v?.toLowerCase() === targets[i]?.nasal.toLowerCase()).length
      ...
      <div className={styles.exerciseWrap}>
        ...
        <p className={`${styles.exerciseText} ${format === 'adapted' ? styles.exerciseTextAdapted : ''}`}>
          {segments.map((seg, i) => {
            ...
            const showHint = hints.has(idx) && typed.toLowerCase() !== seg.nasal.toLowerCase()
            const fullyFilled = typed.toLowerCase() === seg.nasal.toLowerCase()
            ...
            if (phase === 'show') {
              return (
                <span key={i}>
                  <span
                    className={styles.wordWithNasal}
                    onClick={() => playTTS(seg.beforeNasal + seg.nasal + seg.afterNasal)}
                  >
                    {seg.beforeNasal}
                  </span>
                  <span
            ...
            if (phase === 'fade') {
              return (
                <span key={i}>
                  <span>{seg.beforeNasal}</span>
                  {fadeStep === 0 && <span className={styles.highlight} style={{ background: bgColor }}>{seg.nasal}</span>}
                  {fadeStep === 1 && <span className={styles.fadeDots}>{'•'.repeat(seg.nasal.length)}</span>}
                  {fadeStep === 2 && <span className={styles.fadeEmpty}>{'\u00A0'.repeat(seg.nasal.length)}</span>}
                  <span>{seg.afterNasal}</span>
                </span>
              )
            }
            ...
            if (fullyFilled) {
              return (
                <span key={i}>
                  <span>{seg.beforeNasal}</span>
                  <span className={styles.filledNasal} style={{ color: seg.color }}>{typed}</span>
                  <span>{seg.afterNasal}</span>
                </span>
              )
            }
            ...
            const remaining = seg.nasal.slice(typed.length)
            return (
        ...
        {phase === 'show' && (
          <div className={styles.bottomBar}>
            <p className={styles.showHint}>{t.exercises.g4showHint}</p>
            <button className={styles.readyBtn} onClick={startFade}>{t.exercises.g4readyBtn}</button>
          </div>
        )}
